 ...
 * balloon, and the Swiss Ephemeris WASM engine (~570 KiB) — the one dependency
 * big enough to matter — must stay code-split into its own chunk rather than
 * getting pulled into the main bundle every visitor downloads up front. That
 * chunk-count check is what actually verifies "lazy-loaded": if a future
 * change replaced its dynamic `import(...)` with a static one, Vite would
 * inline it into the main chunk and this check would catch it even before the
 * size budget did.
 *
