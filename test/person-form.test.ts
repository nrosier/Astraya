...
import { personFormValidationMessages } from '../src/domain/person-form.messages.js';
import { buildPerson } from '../src/domain/person.js';

const T = personFormValidationMessages.en;
...
describe('validating a draft', () => {
  it('accepts a complete record and returns the moment', () => {
    const { errors, moment } = validateDraft(VEVAY, T);
  ...
  it('requires a name, because it is how the person is found again', () => {
    expect(validateDraft({ ...VEVAY, displayName: '   ' }, T).errors.displayName).toBeDefined();
  });
  ...
  it('rejects a day the month does not have', () => {
    ...
    const errors = validateDraft({ ...VEVAY, date: '1961-02-30' }, T).errors;
    expect(errors.date).toContain('28 days');
    expect(validateDraft({ ...VEVAY, date: '1960-02-29' }, T).errors.date).toBeUndefined();
  });
  ...
  it('applies the Julian leap rule when the date is in the Julian calendar', () => {
    ...
    expect(validateDraft({ ...VEVAY, date: '1900-02-29', calendar: 'julian' }, T).errors.date).toBeUndefined();
    expect(validateDraft({ ...VEVAY, date: '1900-02-29', calendar: 'gregorian' }, T).errors.date).toBeDefined();
    // `auto` reads pre-1582 dates as Julian, the same reform date `resolveCalendar` uses.
    expect(validateDraft({ ...VEVAY, date: '1500-02-29' }, T).errors.date).toBeUndefined();
  });

  it('rejects year zero rather than guessing a convention', () => {
    expect(validateDraft({ ...VEVAY, date: '0000-06-15' }, T).errors.date).toContain('no year 0');
  });

  it('rejects a month outside 1 to 12 and a malformed date', () => {
    expect(validateDraft({ ...VEVAY, date: '1960-13-01' }, T).errors.date).toBeDefined();
    expect(validateDraft({ ...VEVAY, date: '15/06/1960' }, T).errors.date).toBeDefined();
    expect(validateDraft({ ...VEVAY, date: '' }, T).errors.date).toBeDefined();
  });

  it('accepts a leap second but not a 25th hour', () => {
    expect(validateDraft({ ...VEVAY, time: '23:59:60' }, T).errors.time).toBeUndefined();
    expect(validateDraft({ ...VEVAY, time: '24:00' }, T).errors.time).toBeDefined();
    expect(validateDraft({ ...VEVAY, time: '14:60' }, T).errors.time).toBeDefined();
  });
  ...
  it('accepts a missing time only when the time is marked unknown', () => {
    ...
    expect(validateDraft({ ...VEVAY, time: '' }, T).errors.time).toBeDefined();
    const unknown = validateDraft({ ...VEVAY, time: '', timeAccuracy: 'unknown' }, T);
  ...
  it('rejects coordinates outside the globe, at the boundary as well as far past it', () => {
    expect(validateDraft({ ...VEVAY, latitude: '95' }, T).errors.latitude).toBeDefined();
    expect(validateDraft({ ...VEVAY, longitude: '-181' }, T).errors.longitude).toBeDefined();
    ...
    expect(validateDraft({ ...VEVAY, latitude: '90', longitude: '180' }, T).errors).toEqual({});
    expect(validateDraft({ ...VEVAY, latitude: '-90', longitude: '-180' }, T).errors).toEqual({});
    expect(validateDraft({ ...VEVAY, latitude: '90.5' }, T).errors.latitude).toBeDefined();
    expect(validateDraft({ ...VEVAY, longitude: '180.5' }, T).errors.longitude).toBeDefined();
  });
  ...
  it('treats an empty coordinate as missing rather than as zero', () => {
    ...
    const { errors, moment } = validateDraft({ ...VEVAY, latitude: '', longitude: '' }, T);
  ...
  it('treats a half-typed minus sign as missing, not as an error to shout about', () => {
    expect(validateDraft({ ...VEVAY, longitude: '-' }, T).errors.longitude).toBeDefined();
    expect(validateDraft({ ...VEVAY, longitude: '-85.' }, T).errors.longitude).toBeUndefined();
  });
  ...
  it('keeps an offset override of zero, which means UTC', () => {
    ...
    expect(validateDraft({ ...VEVAY, offsetOverride: '0' }, T).moment?.offsetOverrideMinutes).toBe(0);
    expect(validateDraft({ ...VEVAY, offsetOverride: '' }, T).moment?.offsetOverrideMinutes).toBeUndefined();
  });

  it('bounds the offset override outside every real zone but not at twelve hours', () => {
    expect(validateDraft({ ...VEVAY, offsetOverride: '840' }, T).errors.offsetOverride).toBeUndefined();
    expect(validateDraft({ ...VEVAY, offsetOverride: '1200' }, T).errors.offsetOverride).toBeDefined();
    expect(validateDraft({ ...VEVAY, offsetOverride: 'east' }, T).errors.offsetOverride).toBeDefined();
  });

  it('omits optional fields rather than setting them to a default', () => {
    const moment = validateDraft(VEVAY, T).moment;
  ...
  it('carries a zone override through, trimmed', () => {
    expect(validateDraft({ ...VEVAY, zoneOverride: ' America/Indiana/Vevay ' }, T).moment?.zoneOverride).toBe(
  ...
  it('returns no moment whenever anything is wrong', () => {
    ...
    expect(validateDraft({ ...VEVAY, latitude: '95' }, T).moment).toBeUndefined();
  });
});
...
describe('loading a draft from a stored person', () => {
  it('round trips a complete record', () => {
    ...
    expect(validateDraft(draft, T).errors).toEqual({});
  });
  ...
  it('leaves the fields blank for a person whose birth data is missing', () => {
    ...
    expect(validateDraft(draft, T).moment).toBeUndefined();
  });
...
describe('turning a save into operations', () => {
  ...
  it('writes every field on a first save', () => {
    const fields = draftToMutations(ID, VEVAY, EMPTY_DRAFT, T).map((mutation) => mutation.field);
  ...
  it('writes nothing when nothing changed', () => {
    ...
    expect(draftToMutations(ID, VEVAY, VEVAY, T)).toEqual([]);
  });

  it('writes only the field that changed', () => {
    const mutations = draftToMutations(ID, { ...VEVAY, notes: 'from the certificate' }, VEVAY, T);
  ...
  it('writes the birth moment whole, never field by field', () => {
    ...
    const mutations = draftToMutations(ID, { ...VEVAY, time: '14:31:00' }, VEVAY, T);
  ...
  it('writes coordinates as one register', () => {
    const mutations = draftToMutations(ID, { ...VEVAY, latitude: '38.75' }, VEVAY, T);
  ...
  it('clears an offset override with null rather than zero', () => {
    ...
    const mutations = draftToMutations(ID, { ...VEVAY, offsetOverride: '' }, had, T);
  ...
  it('trims what it writes, and ignores a change that is only whitespace', () => {
    expect(draftToMutations(ID, { ...VEVAY, displayName: '  Ada  ' }, VEVAY, T)).toEqual([]);
    expect(draftToMutations(ID, { ...VEVAY, displayName: ' Grace ' }, VEVAY, T)[0]?.value).toBe('Grace');
  });
  ...
  it('refuses to write a draft that does not validate', () => {
    ...
    expect(() => draftToMutations(ID, { ...VEVAY, latitude: '95' }, VEVAY, T)).toThrow(/does not validate/);
  });
});
